...
async function load(){try{const r=await window.storage.get("sc-ops-v2",true);return r?JSON.parse(r.value):[]}catch{return []}}
async function persist(d){try{await window.storage.set("sc-ops-v2",JSON.stringify(d),true)}catch{}}

function uid(){return`SC-${String(new Date().getFullYear()).slice(2)}-${String(Math.floor(Math.random()*900)+100)}`}
...
function Form({go,onNew}){
  ...
  const[f,setF]=useState({client:"",name:"",email:"",taskType:"",desc:"",urgency:"normal",deadline:"",files:""});
  const[done,setDone]=useState(null);
  const[busy,setBusy]=useState(false);
  const u=k=>v=>setF(p=>({...p,[k]:v}));
  const ok1=f.client&&f.name,ok2=f.taskType&&f.desc;
  async function submit(){setBusy(true);const id=uid();const r={id,...f,status:"pendiente",priority:f.urgency,assignee:null,internalNotes:"",corrections:0,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),deliveredAt:null};await onNew(r);setDone(id);setBusy(false)}
    ...
    <div style={{maxWidth:"560px",margin:"0 auto",padding:"32px 20px"}}>
      ...
      {step===3&&<div>
        ...
        <div style={S.card}>
          ...
          <div style={{display:"flex",gap:"10px",marginTop:"8px"}}><Btn onClick={()=>setStep(2)} variant="ghost" style={{flex:1}}>← Atrás</Btn><Btn onClick={submit} disabled={busy||!ok1||!ok2} style={{flex:2}}>{busy?"Enviando...":"Enviar pedido ✓"}</Btn></div>
        </div>
      </div>}
    </div>
  </div>
...
function Dash({go,reqs,setReqs}){
  ...
  async function upd(id,ch){const u=reqs.map(r=>r.id===id?{...r,...ch,updatedAt:new Date().toISOString()}:r);setReqs(u);await persist(u);if(sel?.id===id)setSel(p=>({...p,...ch}))}
...
function Team({go,reqs,setReqs}){
  ...
  async function move(id,s){const u=reqs.map(r=>r.id===id?{...r,status:s,updatedAt:new Date().toISOString()}:r);setReqs(u);await persist(u)}
...
export default function App(){
  ...
  useEffect(()=>{load().then(d=>{setReqs(d||[]);setLoading(false)})},[]);
  async function addReq(r){const u=[r,...reqs];setReqs(u);await persist(u)}
